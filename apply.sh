...
original_author="15620363888"
original_name="wangzhi"
original_urlname="wangzhi"
original_description="Awesome wangzhi created by 15620363888" 
...
rm -rf "${name}"
rm -rf tests
cp -R ${TEMPLATE_DIR}/wangzhi "${name}"
...
then
    ...
    echo "then run 'wangzhi' or 'python -m wangzhi'"
else
    poetry install
    echo "Applied Flask template"
    echo "Ensure you activate your env with 'poetry shell'"
    echo "then run 'wangzhi' or 'python -m wangzhi' or 'poetry run wangzhi'"
fi
